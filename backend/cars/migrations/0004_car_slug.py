...
Three steps rather than one AddField. Adding a unique column straight away would fail
the moment there is more than one existing row, because every one of them would default
to the same empty string.

The intermediate column is explicitly `db_index=False`. SlugField turns an index on by
default, and on Postgres that means a `..._like` index for pattern matching; the later
AlterField to `unique=True` then tries to create that same index again and the migration
dies on `relation "cars_car_slug_85603327_like" already exists`. The index is added once,
by the final operation, where it belongs.
"""
    ...
    operations = [
        migrations.AddField(
            ...
            field=models.SlugField(
                blank=True, default="", max_length=120, db_index=False
            ),
